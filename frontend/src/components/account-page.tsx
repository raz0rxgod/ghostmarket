...
export default function AccountPage() {
  const { email, isAuthenticated, loading, role, isStaff, logout } = useAuth();
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10">
      ...
      {/* Кнопка выхода теперь только здесь — в шапке она была скрыта на
          мобильном (мелкий текст, легко промахнуться пальцем). Полноширинная
          кнопка с крупной областью тапа подходит и для десктопа, и для
          мобильного. */}
      <button
        onClick={() => logout()}
        className="w-full sm:w-auto mt-6 btn-ghost-outline rounded-xl px-6 py-3 text-sm font-medium"
      >
        Se déconnecter
      </button>
    </main>
